...
#### 【運用・システム管理系】
*   **`import_runs`**: バッチ実行履歴（4時間に1回の実行ごとの成否・件数）
*   **`import_document_logs`**: `docID` ごとの処理ステータス（Fetch➔Parse➔Persisted 等）。リトライ制御用。

#### 【マスタデータ系】（CSVから事前インポート）
*   **`edinet_codes`**: 企業マスタ（EDINETコード、証券コード、企業名、業種）
*   **`fund_codes`**: ファンドマスタ（ファンドコード、ファンド名）
